...
class KeyboardModel_backup2 extends CI_Model
{
	...
	public function create($shift, $shift_excel)
	{
		...
		if ($shift_excel['type'] != 'N/A') {
			...
			$date = date('Y-m-d', strtotime($day)); //using the date from the excel file.
			$date_end = date('Y-m-d', strtotime($day_end)); //using the date from the excel file.
